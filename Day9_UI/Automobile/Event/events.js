...
class EventListener {
    ...
    async txnEventListener(role,identityLabel,channelName,chaincodeName,contractName,transactionName,...args){
        let gateway=new Gateway()
        try{
            let userProfile = profile[role.toLowerCase()];
            const cpPath = path.resolve(userProfile["CP"]);
    ...
    async contractEventListener(role,identityLabel,channelName,chaincodeName,contractName,eventName){
        let gateway=new Gateway()
        try{
            let userProfile = profile[role.toLowerCase()];
            const cpPath = path.resolve(userProfile["CP"]);
